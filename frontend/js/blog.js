...
const ARTICLE_SUMMARY_ENABLED = false;
const ARTICLE_COMMENTS_ENABLED = false;
let authorPageName = "";
...
async function resolveAuthorDisplayName(rawAuthor) {
  const fallbackRaw = String(rawAuthor || "").trim();
  if (!fallbackRaw) return "";

  const cacheKey = fallbackRaw.toLowerCase();
  if (authorDisplayNameCache.has(cacheKey)) {
    return authorDisplayNameCache.get(cacheKey);
  }

  let resolved = fallbackRaw;
  try {
    const res = await fetch(`/api/auth/author?name=${encodeURIComponent(fallbackRaw)}`);
    if (res.ok) {
  ...
  if (isEmailLike(resolved)) {
    resolved = "Writer";
  }

  authorDisplayNameCache.set(cacheKey, resolved);
  return resolved;
}
...
function optimizeCloudinaryImageUrl(value, options = {}) {
  ...
  const parsedWidth = Number.parseInt(String(options?.maxWidth ?? ""), 10);
  const maxWidth = Number.isFinite(parsedWidth) && parsedWidth > 0 ? parsedWidth : 960;
  const uploadPathToken = "/image/upload/";
  const transformSegment = `f_auto,q_auto,dpr_auto,c_limit,w_${maxWidth}`;
...
function renderLinkedText(text) {
  ...
  const raw = rawInput.replace(/<(strong|b|em|i)\b[^>]*>([\s\S]*?)<\/\1>/gi, (_, tagName, content) => {
    const normalizedTag = String(tagName || "").toLowerCase() === "b"
      ? "strong"
      : String(tagName || "").toLowerCase() === "i"
        ? "em"
    ...
    const tokenId = `__INLINE_TOKEN_${inlineTokens.length}__`;
    inlineTokens.push(`<${normalizedTag}>${escapeHtml(normalizedContent)}</${normalizedTag}>`);
    return tokenId;
  });
  ...
  inlineTokens.forEach((tokenHtml, index) => {
    const tokenId = `__INLINE_TOKEN_${index}__`;
    html = html.split(tokenId).join(tokenHtml);
  });
...
function renderListItems(items, ordered = false) {
  ...
  const body = sourceItems
    .map((item) => {
      if (item && typeof item === "object" && !Array.isArray(item)) {
        const content = renderLinkedText(item.content || item.text || "");
        const nestedRaw = Array.isArray(item.items) ? item.items : [];
        const nestedHtml = nestedRaw.length ? renderListItems(nestedRaw, ordered) : "";
        if (!content && !nestedHtml) return "";
        return `<li>${content}${nestedHtml}</li>`;
      }

      const content = renderLinkedText(item || "");
      if (!content) return "";
  ...
  if (!body) return "";
  return `<${listTag}>${body}</${listTag}>`;
}

function renderListBlock(data) {
  const style = String(data?.style || data?.type || "unordered").toLowerCase();
  const ordered = style === "ordered" || style === "ol";
  return renderListItems(data?.items, ordered);
}
...
function renderParagraphBlock(text) {
  ...
  if (normalizedLines.length >= 2 && normalizedLines.every(line => /^[-*•]\s+/.test(line))) {
    const bulletItems = normalizedLines.map(line => line.replace(/^[-*•]\s+/, "").trim()).filter(Boolean);
    if (bulletItems.length) {
      return `<ul>${bulletItems.map(item => `<li>${renderLinkedText(item)}</li>`).join("")}</ul>`;
    }
  }

  if (normalizedLines.length >= 2 && normalizedLines.every(line => /^\d+[\.)]\s+/.test(line))) {
    const orderedItems = normalizedLines.map(line => line.replace(/^\d+[\.)]\s+/, "").trim()).filter(Boolean);
    if (orderedItems.length) {
      return `<ol>${orderedItems.map(item => `<li>${renderLinkedText(item)}</li>`).join("")}</ol>`;
    }
  }
...
async function loadPosts() {
  ...
  const res = await fetch("/api/posts?list=1", { cache: "no-store" });
...
async function loadAuthorPage() {
  ...
  const authorSearchInput = document.getElementById("search-input") || document.getElementById("author-search-input");
  const authorSearchResults = document.getElementById("search-results") || document.getElementById("author-search-results");
  const authorClearSearch = document.getElementById("clear-search") || document.getElementById("author-clear-search");
  ...
  const renderAuthorHero = (profile) => {
    ...
    const existingBio = document.getElementById("author-hero-bio");
    const bioText = bio || "This author has not added a bio yet.";
    if (existingBio) {
      existingBio.textContent = bioText;
      existingBio.classList.toggle("bio-placeholder", !bio);
      existingBio.hidden = false;
    } else {
      const bioEl = document.createElement("p");
      bioEl.id = "author-hero-bio";
      bioEl.className = "author-hero-bio" + (bio ? "" : " bio-placeholder");
      bioEl.textContent = bioText;
      const heroContent = heroName?.parentElement;
      if (heroContent && heroSubtitle) {
        heroSubtitle.insertAdjacentElement("afterend", bioEl);
      }
    }
    ...
    if (heroAvatar) {
      ...
      heroAvatar.crossOrigin = "anonymous";
      heroAvatar.onerror = () => {
  ...
  const renderAuthorSections = () => {
    ...
    const localize = (text) => {
      if (typeof localizeTextValue === "function") {
        return localizeTextValue(text);
      }
      return text;
    };
    ...
    const searchQuery = normalizeSearchText(authorSearchInput?.value || "");
    const hasSearchQuery = Boolean(searchQuery);

    const searchFilteredPosts = hasSearchQuery
      ? filteredPosts.filter((post) => {
          const titleMatch = normalizeSearchText(post.title).includes(searchQuery);
          const excerptMatch = normalizeSearchText(post.excerpt).includes(searchQuery);
          const authorMatch = normalizeSearchText(post.author).includes(searchQuery);
          const slugMatch = normalizeSearchText(post.slug).includes(searchQuery);
          const categoryMatch = Array.isArray(post.categories) && post.categories.some(cat =>
            normalizeSearchText(normalizeCategoryLabel(cat)).includes(searchQuery)
          );
    ...
    const latestSorted = getSortedPosts(searchFilteredPosts);
    ...
    if (authorSearchResults) {
      if (hasSearchQuery) {
        const plural = latestSorted.length === 1 ? localize("article") : localize("articles");
        authorSearchResults.innerHTML = `<span style="color: var(--accent);">${latestSorted.length}</span> ${plural} ${localize("found")}`;
      } else {
        authorSearchResults.innerHTML = "";
      }
    }

    if (authorClearSearch) {
      authorClearSearch.style.display = hasSearchQuery ? "flex" : "none";
    }
    ...
    if (hasActiveCategoryFilter || hasSearchQuery) {
      return;
    }
  ...
  if (authorClearSearch && authorSearchInput) {
  ...
  try {
    // Fetch posts first to get authorId for accurate profile lookup
    ...
    // Use authorId from first post for exact user lookup
    const firstAuthorId = list.length > 0 ? (list[0].authorId || "") : "";
    const profileQuery = firstAuthorId
      ? `id=${encodeURIComponent(firstAuthorId)}&name=${encodeURIComponent(author)}`
      : `name=${encodeURIComponent(author)}`;

    const profileResponse = await fetch(`/api/auth/author?${profileQuery}`);
    ...
    renderAuthorHero({ name: author, avatarUrl: "", links: {} });
    header.innerHTML = `<h2>Αναρτήσεις Απο ${escapeHtml(author)}</h2><p>Δεν ήταν δυνατή η φόρτωση αναρτήσεων αυτή τη στιγμή.</p>`;
...
async function findRelatedPosts(currentPost, maxItems = 4) {
  try {
    const response = await fetch("/api/posts?list=1", { cache: "no-store" });
...
async function renderRelatedPosts(currentPost, articleContainer) {
  ...
  const activeLang = document.documentElement.getAttribute("lang") === "en" ? "en" : "el";
  const dateLocale = activeLang === "en" ? "en-US" : "el-GR";
  const relatedTitle = localized("Related Posts");
  const relatedSubtitle = localized("More articles you might like");
  const noImageLabel = localized("No image");

  const section = document.createElement("section");
  section.className = "related-posts";
  section.setAttribute("aria-label", relatedTitle);
  section.innerHTML = `
    <div class="related-posts-head">
      <h2>${escapeHtml(relatedTitle)}</h2>
      <p>${escapeHtml(relatedSubtitle)}</p>
    </div>
  ...
  related.forEach((post) => {
    ...
    card.dataset.hasImage = hasImage ? "true" : "false";
    if (!hasImage) {
      card.classList.add("related-post-card--no-image");
    }

    const safeTitle = escapeHtml(post?.title || "Untitled");
    const safeExcerpt = escapeHtml(getDisplayExcerpt(post, 110));
    const date = new Date(post?.createdAt || Date.now()).toLocaleDateString(dateLocale, {
      year: "numeric",
      month: "short",
      day: "numeric"
    });
    ...
    card.innerHTML = `
      ${hasImage
        ? `<img src="${imageUrl}" alt="${safeTitle}" class="related-post-image" loading="lazy" decoding="async">`
        : ""}
...
function createPostCard(post, options = {}) {
  ...
  const imageUrl = rawImageUrl ? optimizeCloudinaryImageUrl(toSafeHttpUrl(rawImageUrl), { maxWidth: 960 }) : "";
  if (!imageUrl) card.classList.add("no-image");
  ...
  card.innerHTML = `
    ${imageUrl ? `<img src="${imageUrl}" alt="${safeTitle}" class="post-card-image" loading="${imageLoading}" fetchpriority="${imageFetchPriority}" decoding="async">` : ''}
...
async function loadPost() {
  ...
  const resolvedAuthorName = await resolveAuthorDisplayName(post.author);
  const postForDisplay = {
    ...post,
    author: resolvedAuthorName || String(post.author || "").trim()
  };
  ...
  metaParts.push(`<span class="article-reading-time" aria-label="Estimated reading time">⏱ ${estimateReadingMinutesFromHtml(bodyHtml)} min read</span>`);
  ...
  (() => {
    ...
    if (commentsRoot) {
      commentsRoot.hidden = !ARTICLE_COMMENTS_ENABLED;
    }

    const initializeComments = async () => {
      if (!ARTICLE_COMMENTS_ENABLED) return;
    ...
    if (ARTICLE_COMMENTS_ENABLED && commentsRoot && "IntersectionObserver" in window) {
    ...
    } else if (ARTICLE_COMMENTS_ENABLED) {
      window.setTimeout(() => initializeComments(), 1200);
    }

    if (ARTICLE_COMMENTS_ENABLED && "requestIdleCallback" in window) {
...
function createCommentReactionButton(comment, type, label, icon) {
  ...
  button.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setCommentStatus("Δεν ήταν δυνατή η ενημέρωση αντίδρασης.", true);
      ...
      setCommentStatus("Δεν ήταν δυνατή η επικοινωνία με τον διακομιστή.", true);
    } finally {
      button.disabled = false;
    }
  });
...
async function deleteComment(commentId) {
  ...
  if (!res.ok) {
    const status = document.getElementById("commentStatus");
    if (status) status.textContent = "Δεν ήταν δυνατή η διαγραφή σχολίου.";
    return;
  }
...
function setupCommentForm(postId) {
  ...
  form.addEventListener("submit", async e => {
    ...
    if (!res.ok) {
      const payload = await res.json().catch(() => ({}));
      status.textContent = payload.error || "Δεν ήταν δυνατή η δημοσίευση σχολίου. Δοκιμάστε ξανά.";
      status.style.color = "var(--error)";
      return;
    }
...
function initializeNewsletterCapture() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        const payload = await response.json().catch(() => ({}));
        statusEl.textContent = payload?.error || "Δεν ήταν δυνατή η εγγραφή αυτή τη στιγμή.";
      ...
      statusEl.textContent = "Δεν ήταν δυνατή η εγγραφή αυτή τη στιγμή.";
      statusEl.classList.add("is-error");
    }
  });
}
