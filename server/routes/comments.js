...
router.post("/:postId", auth, async (req, res) => {
  ...
  if (!text) return res.status(400).json({ error: "Please enter a comment before posting." });
  if (text.length < 4) {
    return res.status(400).json({ error: "Comment is too short. Please write at least 4 characters." });
  }
  ...
  if (!user) return res.status(404).json({ error: "Account not found. Please log in again." });
